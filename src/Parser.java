public class Parser {
    ...
    public void processAsPageTitle(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (isLetter(arr[i]) || Character.isDigit(arr[i]))
                ...
                this.indexObj.handlerLocal(word.toString(), Settings.TITLE);
                word.setLength(0);
            }
        }
        if (word.length() > 0) {
    ...
    private void processInfoBox(String infoBoxString) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (isLetter(curr)) {
                ...
                this.indexObj.handlerLocal(word.toString(), Settings.INFOBOX);
                word.setLength(0);
            }
        }
        if (word.length() > 0) {
    ...
    private void processCategories(String categoryString) {
        ...
        for (long i = 0; i < len; i++) {
            ...
            if (isLetter(currCh)) {
                ...
                this.indexObj.handlerLocal(word.toString(), Settings.CATEGORY);
                word.setLength(0);
    ...
    /*
     * Function to process stopWords of external reference field data
     */
    ...
    private void processExternal(String links) {
        ...
        for (int j = 0; j < len; j++) {
            ...
            if (links.matches(".*http:.*") || links.matches(".*https:.*")) continue;
                ...
                this.indexObj.handlerLocal(links.trim(), Settings.EXTERNAL_REF);
            }
        }
    }
    ...
    private void processReferences(String links) {
        ...
        for (int j = 0; j < len; j++) {
            ...
            if (links.matches(".*http:.*") || links.matches(".*https:.*")) continue;
                ...
                this.indexObj.handlerLocal(links.trim(), Settings.REFERENCE);
            }
        }
    }
    ...
    public void processAsPageText(String text) {
        ...
        for (int i = 0; i < n; i++) {
             ...
             */
                ...
                this.indexObj.handlerLocal(word.toString(), Settings.BODY);
                word.setLength(0);
            }
        }
