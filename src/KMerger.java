...
public class KMerger {

    public static void mergeTitle(int val) {
        try {
            System.out.println("Merging docID-title dictionary files");
            int isnull = 0;
            long charCount = 0;
            ...
            for (int j = 1; j <= val; j++)
                br2[j] = new BufferedReader(new FileReader(Settings.indexDirec + "IdTitle" + j + ".txt"));
            ...
            File out_file = new File(Settings.indexDirec + "IdTitle.txt");
            ...
            File out_file1 = new File(Settings.indexDirec + "SecTitleIndex.txt");
            if (!out_file1.exists()) {
    ...
    public static void mergeFiles(int val) {
        try {
            System.out.println("Merging and building multi-level indexes");
            int isnull = 0;
            long charCount = 0;
            ...
            for (int j = 1; j <= val; j++)
                br2[j] = new BufferedReader(new FileReader(Settings.fileDirec + "file" + j + ".txt"));
            ...
            File out_file = new File(Settings.indexDirec + "Index.txt");
            ...
            //Secondary Index file
            File out_file1 = new File(Settings.indexDirec + "SecIndex.txt");
            if (!out_file1.exists()) {
                out_file1.createNewFile();
            }
            ...
            //Tertiary Index file
            File out_file2 = new File(Settings.indexDirec + "TerIndex.txt");
            if (!out_file2.exists()) {
            ...
            // Document count file
            File out_file3 = new File(Settings.indexDirec + "DocCount.txt");
            if (!out_file3.exists()) {
                out_file3.createNewFile();
            }
            FileWriter fw3 = new FileWriter(out_file3.getAbsoluteFile());
    ...
    public static String ranking(String postingList) {
        ...
        for (int i = 0; i < recs.length; i++) {
            ...
            tf = (bValue * Settings.BODY_WEIGHT) + (cValue * Settings.CATEGORY_WEIGHT) +
                    (eValue * Settings.EXTERNAL_REF_WEIGHT) + (gValue * Settings.GEOBOX_WEIGHT) +
                    (tValue * Settings.TITLE_WEIGHT) + (rValue * Settings.REFERENCE_WEIGHT);
